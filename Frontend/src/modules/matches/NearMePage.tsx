...
const NearMePage = () => {
  ...
  const { allProfiles, data: currentUserProfile, loading } = useSelector(
  ...
  const filteredMatches = useMemo(() => {
    return allProfiles
      .map((profile) => ({
        ...
        location: [profile.city, profile.state, profile.country].filter(Boolean).join(", ") || "Location not set",
        ...
        compatibility: getCompatibilityScore(profile.userId || profile.id || ""),
      ...
      .filter((match) => {
        // Core "Near Me" Logic: Only show profiles from same city or state as user
        const isNear = !currentUserProfile?.city || 
                       match.rawCity === currentUserProfile.city || 
                       match.rawState === currentUserProfile.state;
  ...
  }, [allProfiles, filters, currentUserProfile]);
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 pb-8 border-b border-slate-100">
          <div className="space-y-2">
            ...
            <p className="text-slate-500 font-medium tracking-wide text-xs">
              Showing {filteredMatches.length} profiles from your city or state.
            </p>
          </div>
        </div>
